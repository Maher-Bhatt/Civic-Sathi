...
import {
  getPlatformStats, listRealWorkOrders, getCachedPlatformStats, getCachedWorkOrders,
  createUser, createRealContractor, listAdminCities,
} from "@/services/shared-store";
import { GlassCard, SectionLabel } from "@/components/ui/glass-card";
import {
  Building2, ClipboardList, Shield, Timer, Users, AlertCircle,
  CheckCircle2, ShieldAlert, Activity, FileText, MapPin, Plus, Copy, Check, Key, UserCheck, X,
} from "lucide-react";
    ...
    <div className="space-y-8 muni-page-enter">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-[var(--muted-foreground)]">Real-time governance, officer provisioning & civic intelligence</p>
        </div>
      ...
      <GlassCard className="p-6 border-blue-500/30 bg-blue-950/10">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-base font-semibold text-[var(--foreground)]">System & Demo Login Credentials</h2>
          </div>
          <span className="text-xs text-[var(--muted-foreground)]">Click copy to test any portal</span>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
          {DEMO_LOGINS.map((item, idx) => (
            <div key={idx} className="p-3.5 rounded-xl bg-[var(--surface-elevated)] border border-[var(--glass-border)] flex flex-col justify-between gap-3">
              <div>
                ...
                <p className="text-xs font-mono font-medium text-[var(--foreground)] truncate select-all">{item.email}</p>
                <p className="text-[11px] font-mono text-[var(--muted-foreground)] mt-0.5">Password: <span className="text-[var(--foreground)] font-semibold">{item.pass}</span></p>
              </div>
              ...
              <div className="flex items-center gap-2 pt-1 border-t border-[var(--glass-border)]/50">
                ...
                >
                  {copiedKey === `email_${idx}` ? <Check className="w-3 h-3 text-emerald-400" /> : <Copy className="w-3 h-3" />}
                  Copy ID
                </button>
                <button
                  ...
                  className="flex-1 flex items-center justify-center gap-1 py-1 rounded bg-[var(--surface)] text-[11px] font-medium text-[var(--muted-foreground)] hover:text-white hover:bg-[var(--glass-border)] transition"
                >
                  {copiedKey === `pass_${idx}` ? <Check className="w-3 h-3 text-emerald-400" /> : <Key className="w-3 h-3" />}
                  Copy Pass
                </button>
              </div>
            </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm">
          <div className="w-full max-w-lg bg-[var(--surface-elevated)] border border-[var(--glass-border)] rounded-2xl p-6 shadow-2xl space-y-4">
            <div className="flex items-center justify-between border-b border-[var(--glass-border)] pb-3">
              ...
              <button onClick={() => setShowOfficerModal(false)} className="text-[var(--muted-foreground)] hover:text-white">
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <form onSubmit={handleCreateOfficer} className="space-y-3">
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-xs font-semibold text-[var(--muted-foreground)]">Full Name *</label>
                  <input
                    type="text"
                    required
                ...
                <div>
                  <label className="text-xs font-semibold text-[var(--muted-foreground)]">Email (Login ID) *</label>
                  <input
                    type="email"
                    required
              ...
              <div className="grid grid-cols-3 gap-3">
                ...
                <div>
                  <label className="text-xs font-semibold text-[var(--muted-foreground)]">Department</label>
                  <select
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  <label className="text-xs font-semibold text-[var(--muted-foreground)]">Phone</label>
                  <input
                    type="text"
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm">
          <div className="w-full max-w-lg bg-[var(--surface-elevated)] border border-[var(--glass-border)] rounded-2xl p-6 shadow-2xl space-y-4">
            <div className="flex items-center justify-between border-b border-[var(--glass-border)] pb-3">
              ...
              <button onClick={() => setShowContractorModal(false)} className="text-[var(--muted-foreground)] hover:text-white">
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <form onSubmit={handleCreateContractor} className="space-y-3">
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-xs font-semibold text-[var(--muted-foreground)]">Company Name *</label>
                  <input
                    type="text"
                    required
                ...
                <div>
                  <label className="text-xs font-semibold text-[var(--muted-foreground)]">Contact Person *</label>
                  <input
                    type="text"
                    required
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-xs font-semibold text-[var(--muted-foreground)]">Company Email *</label>
                  <input
                    type="email"
                    required
              ...
              <div className="grid grid-cols-2 gap-3 pt-2 border-t border-[var(--glass-border)]">
                <div>
                  <label className="text-xs font-semibold text-[var(--muted-foreground)]">Portal Login Email</label>
                  <input
                    type="email"
                    placeholder="Leave empty to use company email"
                    value={contractorForm.login_email}
                    onChange={(e) => setContractorForm({ ...contractorForm, login_email: e.target.value })}
                    className="w-full mt-1 px-3 py-2 rounded-lg bg-[var(--surface)] border border-[var(--glass-border)] text-sm font-mono"
                  />
                </div>
                <div>
                  <label className="text-xs font-semibold text-[var(--muted-foreground)]">Portal Password</label>
                  <input
                    type="password"
                    required
                    value={contractorForm.login_password}
                    onChange={(e) => setContractorForm({ ...contractorForm, login_password: e.target.value })}
                    className="w-full mt-1 px-3 py-2 rounded-lg bg-[var(--surface)] border border-[var(--glass-border)] text-sm font-mono"
                  />
                </div>
              </div>
