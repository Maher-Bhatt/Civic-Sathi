  ...
  return (
    <div className="relative inline-block">
      <button
        ...
        title={isIos ? "Install Civic Sathi on iPhone or iPad" : "Install Civic Sathi on Android or desktop"}
      ...
      {showHelp && (
        <div role="dialog" aria-label="Install Civic Sathi" className="absolute right-0 top-full z-[60] mt-2 w-72 rounded-2xl border border-[var(--glass-border)] bg-[var(--glass-strong)] p-4 text-left shadow-[var(--shadow-lift)] backdrop-blur-2xl">
          <div className="flex items-start gap-3">
            ...
            <div>
              <p className="text-sm font-semibold text-foreground">Use Civic Sathi like an app</p>
              <p className="mt-1 text-xs leading-5 text-muted-foreground">
                {isIos
                  ? "In Safari, tap Share → Add to Home Screen → turn on Open as Web App → Add."
                  : "Choose Install Civic Sathi or Add to Home screen from your browser menu."}
              </p>
            </div>
          </div>
          <p className="mt-3 flex items-center gap-1.5 text-[0.68rem] text-subtle"><ExternalLink className="h-3 w-3" aria-hidden /> Secure install; live civic data stays online.</p>
        </div>
      )}
    </div>
  );
