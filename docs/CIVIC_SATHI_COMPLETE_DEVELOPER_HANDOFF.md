...
| 045 | 🟠 High | `apps/public/src/services/cities.ts`, `location-picker.tsx` | GPS detection accepted out-of-boundary coordinates worldwide, submitting invalid complaints outside supported municipal zones | Added municipal bounding radii (Vadodara 25km, Mumbai 40km, Bengaluru 30km, Delhi 35km) and Haversine distance verification to block out-of-city submissions |
| 046 | 🔴 Critical | `backend/app/services/ai_service.py` | Groq decommissioned cloud vision models; initial color-histogram classifier had false positives (e.g. pothole -> street lighting) | Engineered multi-signal vision diagnostic engine extracting luminance, saturation, and Sobel roughness synthesized with Groq LLM (`allam-2-7b`) and deterministic rule-based fallback |
| 047 | 🔴 Critical | `backend/app/repositories/complaint_repository.py` | PostgreSQL sequence `complaint_public_seq` fell behind seeded rows (118,958 vs 142,950+), throwing UniqueViolation 500 error on every public submission | Resynced Neon database sequence and made `get_next_public_id_number` self-healing (`GREATEST(nextval, max + 1)` and automatic `setval`) |
| 048 | 🔴 Critical | `backend/app/api/v1/routes/complaints.py` | `create_complaint` route required strict auth (`Depends(get_current_user)`), rejecting anonymous and expired token submissions with 403 Forbidden | Switched dependency to `Depends(get_optional_user)` so public citizen reports succeed for both logged-in and guest citizens |
| 049 | 🟡 Medium | `backend/app/api/v1/routes/auth.py` | Citizen `/login` route filtered exclusively for `citizen` and `contractor` roles, rejecting administrative accounts with `Invalid email or password` | Removed role exclusion so administrative accounts (e.g. `maherbhatt01@gmail.com`) can authenticate on the public citizen portal |
